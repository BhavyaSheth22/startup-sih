...
const GiveEquityModal = (close) => {
	...
	const submit = async () => {
		...
		const toastElement = toast.loading("Submitting Offer");
		try {
	...
	return (
		<div className="bg-gray-100 rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0 modal">
			<Input
				label="Precentage of Equity to offer"
				type="text"
				setter={setEquityPercentage}
			/>
			<Input
				label="Price for 1% equity"
				type="text"
				setter={setPriceForOnePercent}
			/>
			<button
				onClick={submit}
